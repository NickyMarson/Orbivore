...
def drawOptionList(title, options, selected_index, y_start, spacing, color_selected_bg, color_selected_text, color_unselected_text):
    pyxel.text(centerTextHorizontal(title), 50, title, 7)

    for i, option in enumerate(options): # Enumerate over the options array and draw all options dynamically
        x = centerTextHorizontal(option)
        y = y_start + i * spacing
...
class App:
    ...
    def drawMenu(self): # Draws menu
        ...
        y_start=100, spacing=20, color_selected_bg=6, color_selected_text=0, color_unselected_text=7)
